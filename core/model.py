...
class A3CGazModel(Model):
    def __init__(self, args):
        super(A3CGazModel, self).__init__(args)
        # Actor Net
        self.conv1 = nn.Conv2d(self.input_dims[0], 32, kernel_size=3, stride=2) # NOTE: for pkg="atari"
        self.rl1   = nn.ReLU()
        self.conv2 = nn.Conv2d(32, 32, kernel_size=3, stride=2, padding=1)
        ...
        self.tanh = nn.Tanh()
        if self.enable_lstm:
            self.lstm  = nn.LSTMCell(4*5*32+4, self.hidden_dim, 1)
        self.policy_5 = nn.Linear(self.hidden_dim, self.output_dims)
        self.policy_sig = nn.Linear(self.hidden_dim, self.output_dims)
        self.softplus = nn.Softplus()

        # Critic net
        self.conv1_v = nn.Conv2d(self.input_dims[0], 32, kernel_size=3, stride=2) # NOTE: for pkg="atari"
        self.rl1_v   = nn.ReLU()
        self.conv2_v = nn.Conv2d(32, 32, kernel_size=3, stride=2, padding=1)
        self.rl2_v   = nn.ReLU()
        self.conv3_v = nn.Conv2d(32, 32, kernel_size=3, stride=2, padding=1)
        self.rl3_v   = nn.ReLU()
        self.conv4_v = nn.Conv2d(32, 32, kernel_size=3, stride=2, padding=1)
        self.rl4_v   = nn.ReLU()
        if self.enable_lstm:
            self.lstm_v  = nn.LSTMCell(4*5*32+4, self.hidden_dim, 1)
        self.value_5  = nn.Linear(self.hidden_dim, 1)
    ...
    def _init_weights(self):
        ...
        self.value_5.weight.data = normalized_columns_initializer(self.value_5.weight.data, 1.0)
        self.value_5.bias.data.fill_(0)
        self.policy_sig.weight.data = normalized_columns_initializer(self.policy_sig.weight.data, 0.01)
        self.policy_sig.bias.data.fill_(0)
    ...
    def forward(self, x, lstm_hidden_vb=None):
        # depth input
        p = x[0].view(-1, self.input_dims[0], self.input_dims[1][0], self.input_dims[1][1])
        p = self.rl1(self.conv1(p))
        p = self.rl2(self.conv2(p))
        p = self.rl3(self.conv3(p))
        p = self.rl4(self.conv4(p))
        p = p.view(-1, 4*5*32)
        p = torch.cat((p, x[1]), 1)
        if self.enable_lstm:
        ...
        p_mu = self.policy_5(p)
        p1, p2 = torch.split(p_mu,1,1)
        p1 = F.sigmoid(p1)
        p2 = F.tanh(p2)
        p_out= torch.cat((p1, p2), 1)
        sig = self.policy_sig(p)
        sig = self.softplus(sig)

        v = x[0].view(-1, self.input_dims[0], self.input_dims[1][0], self.input_dims[1][1])
        v = self.rl1_v(self.conv1_v(v))
        v = self.rl2_v(self.conv2_v(v))
        v = self.rl3_v(self.conv3_v(v))
        v = self.rl4_v(self.conv4_v(v))
        v = v.view(-1, 4*5*32)
        v = torch.cat((v, x[1]), 1)
        if self.enable_lstm:
